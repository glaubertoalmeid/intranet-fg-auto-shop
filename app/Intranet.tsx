...
export default function Intranet() {
  ...
  const [deepLink,setDeepLink] = useState<{productId?:number}>({});
...
type SearchResults={products:SearchProduct[];tasks:SearchTask[]};
const emptySearch:SearchResults={products:[],tasks:[]};

/** Busca global (Ctrl+K) na barra superior — produtos e tarefas, cada seção só aparece
 *  se o resultado da API trouxer algo (a API já filtra por permissão do usuário). Clicar
 *  num resultado navega direto pro item. */
function GlobalSearch({onProduct,onTask}:{onProduct:(id:number)=>void;onTask:()=>void}){
 ...
 const hasResults=results.products.length||results.tasks.length;
